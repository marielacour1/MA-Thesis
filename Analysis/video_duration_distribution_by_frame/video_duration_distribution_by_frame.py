...
def load_durations_by_frame() -> dict[str, list[float]]:
    durations_by_frame: dict[str, list[float]] = defaultdict(list)

    with INPUT_CSV.open("r", encoding="utf-8-sig", newline="") as f:
        reader = csv.DictReader(f)
        fieldnames = reader.fieldnames or []
        missing = [column for column in (FRAME_COLUMN, DURATION_COLUMN) if column not in fieldnames]
        if missing:
            raise ValueError(f"Missing required columns: {', '.join(missing)}")

        for row in reader:
            frame = (row.get(FRAME_COLUMN) or "").strip()
            duration_seconds = parse_duration_seconds(row.get(DURATION_COLUMN))
            if not frame or duration_seconds is None:
                continue
            durations_by_frame[frame].append(duration_seconds)

    if not durations_by_frame:
        raise ValueError("No valid frame/duration rows found.")
    return dict(durations_by_frame)


def make_log_bins(values: list[float], bin_count: int = 45) -> list[float]:
    min_value = max(min(values), 1 / 60)
...
def frame_order(durations_by_frame: dict[str, list[float]]) -> list[str]:
    counts = Counter({frame: len(durations) for frame, durations in durations_by_frame.items()})
...
def draw_panel(
    ...
    _, _, small_font, note_font, bold_font = fonts
    ...
    percentages = histogram_percentages(durations, bins)
    ...
    draw.text(
        (right - 210, top + 2),
        f"n = {len(durations):,}\nmean = {avg:.1f} sec\nmiddle 50 mean = {stats['middle_50_mean']:.1f} sec\nmedian = {median:.1f} sec\np95 = {p95:.1f} sec",
        fill="#333333",
        font=note_font,
        spacing=3,
    )
    ...
    median_x = plot_left + (math.log10(median) - log_min) / (log_max - log_min) * plot_width
    mean_x = plot_left + (math.log10(avg) - log_min) / (log_max - log_min) * plot_width
    ...
    tick_values = [10, 60, 600, 3600, 36000]
    for tick in tick_values:
        if tick < bins[0] or tick > bins[-1]:
            continue
        x = plot_left + (math.log10(tick) - log_min) / (log_max - log_min) * plot_width
        draw.line((x, plot_bottom, x, plot_bottom + 5), fill="#333333", width=1)
        label = format_duration_tick(tick)
...
def save_duration_multiplot(durations_by_frame: dict[str, list[float]]) -> None:
    frames = frame_order(durations_by_frame)
    all_durations = [duration for durations in durations_by_frame.values() for duration in durations]
    bins = make_log_bins(all_durations)
    max_percentage = max(
        max(histogram_percentages(durations, bins))
        for durations in durations_by_frame.values()
    )

    width, height = 1800, 1500
    image = Image.new("RGB", (width, height), "white")
    draw = ImageDraw.Draw(image)
    title_font, subtitle_font, small_font, note_font, _ = load_fonts()
    fonts = load_fonts()

    draw.text((70, 42), "Distribution of Video Durations Across Frames", fill="#1f1f1f", font=title_font)
    draw.text(
        (70, 98),
        "Duration is shown in seconds on a log scale; y-values are percentages within each frame.",
        fill="#555555",
        font=small_font,
    )
    ...
    for idx, frame in enumerate(frames):
        ...
        draw_panel(draw, box, frame, durations_by_frame[frame], bins, max_percentage, fonts)

    draw.text((790, 1440), "Video duration in seconds (log scale)", fill="#333333", font=subtitle_font)
...
def build_descriptive_table(durations_by_frame: dict[str, list[float]]) -> str:
    rows = []
    for frame in frame_order(durations_by_frame):
        stats = descriptive_stats(durations_by_frame[frame])
        rows.append(
            {
                "Frame": frame,
                "n": f"{int(stats['n']):,}",
    ...
    columns = ["Frame", "n", "Mean", "Middle 50 mean", "SD", "Min", "Q1", "Median", "Q3", "P95", "Max"]
    widths = {
    ...
    lines = [
        "Video duration descriptive statistics by frame (seconds)",
...
def save_descriptive_table(durations_by_frame: dict[str, list[float]]) -> str:
    table = build_descriptive_table(durations_by_frame)
    OUTPUT_STATS_TXT.write_text(table, encoding="utf-8")
    return table


def main() -> None:
    durations_by_frame = load_durations_by_frame()
    save_duration_multiplot(durations_by_frame)
    table = save_descriptive_table(durations_by_frame)

    print(f"Input CSV: {INPUT_CSV}")
    print(f"Saved duration multiplot to: {OUTPUT_PNG}")
